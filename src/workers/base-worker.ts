...
import { ReportJobMessage } from '../shared/types/sqs.types.js';
import { MountNotAvailableError } from '../shared/errors/app-errors.js';
...
export class BaseWorker {
  ...
  async processRecord(record: SQSRecord): Promise<void> {
    ...
    try {
      ...
      const engine = this.engineRegistry.resolve(job.format as ReportFormat);
      await engine.generate({ job, template, templatePath, outputPath });

      const ext = getExtension(job.format as ReportFormat);
